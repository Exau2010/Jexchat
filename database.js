...
// Nettoyage groupé des tokens FCM devenus invalides/expirés (retournés par
// Firebase après un envoi) : une seule opération $pull avec $in, jamais une
// requête par token.
async function removeFcmTokens(userId, tokens) {
  if (!tokens || tokens.length === 0) return null;
  return User.updateOne({ _id: userId }, { $pull: { fcmTokens: { $in: tokens } } });
}
...
// Récupération groupée par lot (évite le N+1 lors de l'enrichissement des
// réponses à un message : une seule requête pour toutes les réponses de la
// page au lieu d'une par message).
async function getMessagesByIds(ids) {
  return Message.find({ _id: { $in: ids } }).lean();
}
...
async function getUserNotifications(userId, skip = 0, limit = 20) {
  // Seules les notifications non lues sont listées : une fois lue, une
  // notification disparaît réellement de la liste (voir routes /read et
  // /read-all), donc inutile de re-fetcher les anciennes déjà traitées.
...
// Corbeille : suppression définitive et groupée (une seule deleteMany) de
// toutes les notifications de l'utilisateur authentifié. Distinct de
// markAllNotificationsRead : "lu" et "supprimé" sont deux opérations
// différentes, celle-ci ne doit jamais être déclenchée par un simple marquage
// comme lu.
...
module.exports = {
  ...
  addFcmToken, removeFcmToken, removeFcmTokens, deleteUserAccount,
  ...
  createMessage, getMessages, getMessageById, getMessagesByIds, editMessage,
  ...
  markAllNotificationsRead, getUnreadNotificationCount, deleteAllNotifications,
