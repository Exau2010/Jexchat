...
const messageSchema = new mongoose.Schema({
  ...
  type: {
    type: String,
    enum: ['text', 'link', 'image', 'video', 'audio', 'sticker', 'note', 'expired'],
    default: 'text',
  },
...
async function getOnlineUsers(limit = 30) {
  ...
  return User.find({ lastSeen: { $gte: cutoff }, isBanned: { $ne: true } })
    .select('username identity isOnline lastSeen')
    .sort({ lastSeen: -1 })
    .limit(limit)
    .lean();
...
module.exports = {
  connect,
  User, Session, Conversation, Message, Notification, Report,
  ...
  promoteGroupAdmin, demoteGroupAdmin, updateGroupName,
