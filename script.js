...
function initSocket(token) {
  ...
  socket.on('user_offline', ({ userId }) => {
    ...
    updatePresenceDots(userId, false);
  });
...
// ─── Presence ──────────────────────────────────────────────────────────────────
function updatePresenceDots(userId, online) {
...
async function openConversation(conv) {
  ...
  if (chatHeader) {
    ...
    info.innerHTML = `
      ...
      <div class="chat-header-status${isOnline ? ' online' : ''}">${isOnline ? 'En ligne' : 'Hors ligne'}</div>`;
...
async function handleSearch(q) {
  ...
  try {
    ...
    users.forEach(user => {
      ...
      el.innerHTML += `<div class="conv-info"><div class="conv-name">${escText(user.username)}</div><div class="conv-preview">${state.onlineUsers.has(user.id) ? 'En ligne' : 'Hors ligne'}</div></div>`;
