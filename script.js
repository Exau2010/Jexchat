...
function createMessageEl(msg, sender, isOut) {
  ...
  // Reply reference
  if (msg.replyTo) {
    const replyRef = createReplyRef(msg.replyTo);
...
function createReplyRef(replyTo) {
  const ref = document.createElement('div');
  ref.className = 'msg-reply-ref';
  if (typeof replyTo === 'object' && replyTo) {
    ref.innerHTML = `<div class="msg-reply-ref-name">${escText(replyTo.senderId || 'Message')}</div><div class="msg-reply-ref-text">${escText(replyTo.content || replyTo.type || '')}</div>`;
  } else {
...
function openGroupModal(conv) {
  ...
  overlay.innerHTML = `
    <div class="modal">
      ...
      <div class="modal-body">
        ...
        <div class="input-group">
          <label class="input-label">Membres (${conv.members?.length || 0})</label>
          <div>
            ${(conv.members || []).map(m => `
              <div class="admin-table-row" style="border-radius:8px;margin-bottom:4px">
                ${createAvatarEl(m, 'sm').outerHTML}
                <div class="admin-row-info">
                  <div class="admin-row-name">${escText(m.username)}</div>
                </div>
                ${m.id !== conv.owner && m.id !== state.currentUser?.id ? `<button class="btn btn-ghost btn-sm" onclick="removeMember('${conv.id}','${m.id}',this)">Retirer</button>` : ''}
              </div>`).join('')}
          </div>
        </div>
      </div>
  ...
  document.getElementById('group-modal-close')?.addEventListener('click', () => { overlay.classList.remove('open'); setTimeout(() => overlay.remove(), 200); });
  overlay.addEventListener('click', e => { if (e.target === overlay) { overlay.classList.remove('open'); setTimeout(() => overlay.remove(), 200); } });
...
async function toggleNotifPanel() {
  ...
  if (!panel) {
    ...
    document.getElementById('sidebar-header')?.appendChild(panel);
  }
...
async function initMessagesPage() {
  ...
  try {
    ...
    state.blockedUsers = new Set(blocked.map(u => u.id));
  } catch {}

  const count = await api('GET', '/api/notifications/count');
...
// Panneau séparé de la liste des utilisateurs. L'icône dans la barre horizontale
// (inchangée) bascule entre les deux ; aucune reconstruction de page complète.
function togglePublicationsPanel() {
  const feedPanel = document.getElementById('publications-panel');
  const onlineWrap = document.getElementById('online-strip-wrap');
  const convList = document.getElementById('conv-list');
  const btn = document.getElementById('publications-btn');
  if (!feedPanel) return;
  state.posts.open = !state.posts.open;
  if (state.posts.open) {
    feedPanel.style.display = 'block';
    onlineWrap.style.display = 'none';
    convList.style.display = 'none';
    btn.innerHTML = iconUsers();
    btn.title = 'Utilisateurs';
    ...
    btn.innerHTML = iconMsg();
    btn.title = 'Publications';
  }
}
...
function renderPostCard(post) {
  ...
  shareBtn.addEventListener('click', () => sharePostAction(post.id));
...
async function sharePostAction(postId) {
  const description = (prompt('Ajouter un commentaire au partage (optionnel, 300 caractères max) :') || '').substring(0, 300);
  try {
    await api('POST', `/api/posts/${postId}/share`, { description });
