...
async function toggleNotifPanel() {
  ...
  // Ouvrir le panneau se contente d'afficher les notifications non lues :
  // elles restent visibles et ne sont plus marquées comme lues à ce moment
  // (voir closeNotifPanel, appelé à la fermeture).
  ...
  notifPanelOutsideHandler = (e) => {
    if (!panel.contains(e.target) && !e.target.closest('#notif-btn')) closeNotifPanel(panel);
  };
  setTimeout(() => document.addEventListener('click', notifPanelOutsideHandler), 100);
}

// Fermer le panneau marque comme lues, côté serveur, les notifications qui
// étaient affichées (une seule requête groupée) : c'est la fermeture, et non
// plus l'ouverture, qui fait passer les notifications visibles en "lu". Elles
// ne réapparaissent alors plus comme non lues (updateMany déjà en place côté
// serveur pour /api/notifications/read-all).
async function closeNotifPanel(panel) {
  if (!panel) return;
  panel.classList.remove('open');
  if (notifPanelOutsideHandler) {
    document.removeEventListener('click', notifPanelOutsideHandler);
    notifPanelOutsideHandler = null;
  }
  if (state.notifCount > 0) {
    state.notifCount = 0;
    updateNotifBadge();
  ...
  try {
    ...
    messaging.onMessage?.((payload) => {
      ...
      swReg.showNotification(title || 'JEXCHAT', {
        ...
        // Voir firebase-messaging-sw.js : /icon-192.png et /badge-72.png
        // n'existent pas et ne sont servis nulle part, /logo.png est la
        // seule image réellement disponible.
        icon: '/logo.png',
        badge: '/logo.png',
