...
function initSocket(token) {
  ...
  socket.on('notifications_read', ({ notificationId, all, count }) => {
    ...
    if (all) {
      ...
      panel.querySelector(`.notif-item[data-notif-id="${notificationId}"]`)?.classList.remove('unread');
    }
  });
...
function buildVideoPlayer(url, { maxWidth } = {}) {
  ...
  if (info.type === 'youtube' || info.type === 'vimeo') {
    ...
    preview.style.cssText = 'position:relative;cursor:pointer;border-radius:10px;overflow:hidden;background:#000;aspect-ratio:16/9;display:flex;align-items:center;justify-content:center';
    if (info.thumbnail) {
      const img = document.createElement('img');
...
async function renderNotifList(panel) {
  ...
  try {
    ...
    notifs.forEach(n => {
      ...
      el.addEventListener('click', async () => {
        // Marque comme lue avant tout retrait visuel : elle ne disparaît de
        // la liste des non-lues qu'une fois l'état réellement enregistré.
        if (!n.read) {
          try {
