...
// Serve static files (CSS, JS frontend, SW)
const staticFiles = ['/style.css', '/script.js', '/firebase-messaging-sw.js', '/logo.png'];
...
// de template.
app.get('/lapage/:pseudo', (req, res) => res.sendFile(path.join(__dirname, 'messages.html')));
app.get('/lapage/:pseudo/actu/:postId', (req, res) => res.sendFile(path.join(__dirname, 'messages.html')));
...
app.get('/api/users/by-username/:username', sec.requireAuth, async (req, res) => {
  try {
...
app.get('/api/users/:id', sec.requireAuth, async (req, res) => {
  try {
...
app.delete('/api/conversations/:id', sec.requireAuth, async (req, res) => {
  try {
    ...
    if (!conv || conv.type !== 'group') return res.status(404).json({ error: 'Groupe introuvable.' });
    const isOwner = conv.owner?.toString() === req.user._id.toString();
    if (!isOwner) return res.status(403).json({ error: 'Seul le propriétaire peut supprimer le groupe.' });
...
app.post('/api/conversations/:id/messages', sec.requireAuth, sec.messageLimiter, async (req, res) => {
  try {
    ...
    const [msgOutBase] = safeSharedPostId ? await enrichMessages([message]) : [safeMessage(message)];
    const msgOut = {
      ...msgOutBase,
    ...
    for (const memberId of conv.members) {
      ...
      if (mid !== req.user._id.toString()) {
        ...
        if (member && member.settings?.notifications !== false) {
...
app.get('/api/posts/user/:id', sec.requireAuth, async (req, res) => {
  try {
    if (!sec.isValidObjectId(req.params.id)) return res.status(400).json({ error: 'ID invalide.' });
    const skip = parseInt(req.query.skip) || 0;
    const limit = Math.min(parseInt(req.query.limit) || 20, 30);
...
// cette publication, jamais tout le fil.
app.get('/api/posts/:id', sec.requireAuth, async (req, res) => {
  try {
    if (!sec.isValidObjectId(req.params.id)) return res.status(400).json({ error: 'ID invalide.' });
...
// contenu dupliqué) pour les messages de type "shared_post", par lot afin
// d'éviter toute requête N+1.
async function enrichMessages(messages) {
  ...
  const posts = await db.getPostsByIds(sharedIds);
  const authorIds = [...new Set(posts.map((p) => p.authorId.toString()))];
  const authors = authorIds.length ? await db.findUsersByIds(authorIds) : [];
  const authorMap = new Map(authors.map((a) => [a._id.toString(), a]));
  const postMap = new Map(posts.map((p) => [p._id.toString(), p]));
  return base.map((m) => {
    if (!m.sharedPostId) return m;
