...
app.get('/api/notifications', sec.requireAuth, async (req, res) => {
  try {
    ...
    // Mappe _id -> id (comme pour les autres ressources de l'API) : sans ce
    // mapping le client recevait un id `undefined`, si bien que le marquage
    // "lu" d'une notification échouait silencieusement côté serveur et la
    // notification restait donc non lue, se réaccumulant à chaque ouverture.
...
// Suppression définitive des notifications de l'utilisateur connecté
// uniquement : le filtre utilise toujours req.user._id (identité de la
// session authentifiée), jamais un userId fourni par le client.
app.delete('/api/notifications', sec.requireAuth, async (req, res) => {
  try {
    await db.deleteAllNotifications(req.user._id);
    // Réutilise l'événement de synchro existant : mêmes effets côté client
    // qu'un "Tout lire" (liste vidée, compteur à 0) sur tous les onglets/
    // appareils, sans introduire de nouveau canal Socket.IO.
    io.to(`user:${req.user._id}`).emit('notifications_read', { all: true, count: 0 });
    return res.json({ ok: true, count: 0 });
  } catch (err) {
    return res.status(500).json({ error: 'Erreur.' });
